...
Now that we have defined custom exception handlers, let's try to divide a number by zero using the /calculate API call.
...
Similarly, let's pass values to the API call that causes the ValueError exception: 
